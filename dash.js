...
db.collection("interims").orderBy("id").get().then(function(qS) {
    ...
    qS.forEach(function(doc) {
        ...
        $(".card-columns").append("<div class = 'card tripCard' tabindex = '0' ontouchstart = '' id = 'tripCard" + id + "'><div class = 'card-header'><div class = 'card-text toggleStar d-inline-block' id = 'toggleStar" + id + "' tabindex = '0' style = 'margin-bottom: 0 !important'>" + starEmpty + "</div></div><div class = 'card-body trip'><h4 class = 'card-title'>" + (trip.new ? "<span class = 'h4' style = 'color: red'>NEW </span>" : "") + trip.name + "</h6><h6 class = 'card-text'>" + categories + "</h6><h6 class = 'card-text'>HKD" + price + "</h6><p class = 'card-text mb-0'>" + trip.description + "</p></div></div>");
        ...
        $("#toggleStar" + id).click(function(event) {
            ...
            if (!currentStarred[id - 1]) {
                $("#toggleStar" + id).html(starFilled);
                currentStarred[id - 1] = true;
           } else {
        ...
        storage.ref("previews/" + id + ".jpg").getDownloadURL().then(function(url) {
            $("#tripCard" + id).prepend("<div class = 'starHolder toggleStar' id = 'toggleStar" + id + "' tabindex = '0'>" + starEmpty + "</div>");
